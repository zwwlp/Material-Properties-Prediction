...
class Metric(object):
    ...
    def add_batch_metric(self, predictoins, targets):
        if self.name=='MSELoss':
            metric_tensor = torch.sqrt(self.metric_fn(predictoins, targets))
        else:
...
class History(object):
    def __init__(self,file_path="../save_data/history.csv"):
...
class Model(object):
    def __init__(self, device, model, optimizer, scheduler, clip_value=None,
                 metrics=[('MSELoss', nn.MSELoss()), ('MAE', nn.L1Loss())]):
    ...
    def train(self, train_dl, val_dl, num_epochs):
        ...
        for epoch in range(num_epochs-500):
    ...
    def evaluate(self, dataloader):
        ...
        return all_outputs, all_targets,total_metrics
